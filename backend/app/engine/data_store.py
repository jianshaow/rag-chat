...
class DataConfigs(RootModel[Dict[str, DataConfig]]):
    ...
    def update(self, other: Dict[str, DataConfig]):
        self.root.update(DataConfigs.model_validate(other))


DEFAULT_DATA_CONFIG = DataConfig(starter_question="What is the document about?")


__builtin_data_config = DataConfigs.model_validate(
    {
        "en_novel": DataConfig(starter_question="What did the author do growing up?"),
        "zh_novel": DataConfig(starter_question="地球发动机都安装在哪里？"),
    }
)
...
if data_config_file and os.path.isfile(data_config_file):
    with open(data_config_file, "r", encoding="utf-8") as file:
        __data_configs.update(yaml.safe_load(file))
...
def get_starter_question(data_dir):
    return get_data_config()[data_dir].starter_question
